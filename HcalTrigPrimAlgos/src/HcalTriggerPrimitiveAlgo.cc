...
//  addFG(const HcalTrigTowerDetId& id, std::vector<bool>& msb);
...
void HcalTriggerPrimitiveAlgo::addSignal(const HFDataFrame & frame) {
   if(frame.id().depth() == 1 || frame.id().depth() == 2) {
      ...
      //      uint32_t fgid = (frame.id().maskDepth()); //changed to next line P.S
      uint32_t fgid = (frame.id().depth());
...
void HcalTriggerPrimitiveAlgo::analyze(IntegerCaloSamples & samples, HcalTriggerPrimitiveDigi & result) {
  ...
  if(peakfind_) {
    if((shift < shrink) || 
       (shift + tpSamples + shrink > dgSamples - (peak_finder_algorithm_ - 1) )    )
      {
